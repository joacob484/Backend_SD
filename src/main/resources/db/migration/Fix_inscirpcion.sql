-- V3__Mejoras_Inscripcion.sql
-- Mejoras a la tabla de inscripciones

-- 1. Agregar nuevas columnas a la tabla inscripcion
...
-- 2. Actualizar el constraint del estado para incluir CANCELADO
ALTER TABLE inscripcion 
...
-- 4. Renombrar el constraint único si existe con otro nombre
ALTER TABLE inscripcion 
DROP CONSTRAINT IF EXISTS inscripcion_partido_id_usuario_id_key;

-- Crear el constraint con el nombre correcto
ALTER TABLE inscripcion 
...
-- 8. Vista para estadísticas de inscripciones por partido
...
-- 9. Vista para inscripciones activas (PENDIENTE o ACEPTADO)
...
INNER JOIN partido p ON i.partido_id = p.id
...
-- 10. Índice compuesto para búsquedas comunes
...
-- 11. Estadísticas iniciales (opcional - comentar si no se desea)
-- Esto ayuda al query planner de PostgreSQL
ANALYZE inscripcion;